...
public class GlobalExceptionHandler {
	...
	@ExceptionHandler(MethodArgumentNotValidException.class)
		...
		ResponseEntity<Map<String, List<String>>> handleValidationError(MethodArgumentNotValidException e)
		{
			List<String> error = e.getBindingResult()
					.getFieldErrors().stream()
					.map(FieldError::getDefaultMessage)
					.collect(Collectors.toList());
			return new ResponseEntity<Map<String,List<String>>>(getErrorMap(error), new HttpHeaders(),HttpStatus.BAD_REQUEST);
		}
